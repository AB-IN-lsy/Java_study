...
public class StringTest {
    ...
    private static void getManipulating() {
        ...
        System.out.println(simple.substring(6)); //World,一个参数时直接到最后
        System.out.println(simple.substring(0, 5)); // Hello

        String fileName = "Lecture01.pptx";
        int index = fileName.lastIndexOf("."); //从后往前查，但索引还是从前往后的索引
    ...
    private static void getReplace() {
        ...
        String fString = dString.replaceAll("/", "-").replaceAll("：", ":"); //可以叠加使用
    ...
    private static void getStringBuilder() {
        StringBuilder builder = new StringBuilder(); //可变的对象
        for (int i = 0; i < 100; i++) {
            builder.append("学号：" + i + "; "); //始终为一个对象
        }
        System.out.println(builder.toString()); //将字符序列转为字符串
    }
